...
void init_sous_menu_joueur(sous_menu_joueur *sm) {
    ...
    *sm = (sous_menu_joueur){0};
    ...
    sm->retrn_btn[0] = IMG_Load("retrn2.png");
    if (!sm->retrn_btn[0]) {
        fprintf(stderr, "Erreur de chargement de retrn1.png: %s\n", IMG_GetError());
        exit(EXIT_FAILURE);
    }
    sm->retrn_btn[1] = IMG_Load("retrn1.png");
    if (!sm->retrn_btn[1]) {
        fprintf(stderr, "Erreur de chargement de retrn2.png: %s\n", IMG_GetError());
        exit(EXIT_FAILURE);
    }
...
/////////
    if (touche_souris(sm->pos_btn_retrn, event)) {
        if (sm->retrn_btn[1]) SDL_BlitSurface(sm->retrn_btn[1], NULL, ecran, &sm->pos_btn_retrn);
        if (event.type == SDL_MOUSEBUTTONDOWN) {
           
        }
    } else {
        if (sm->retrn_btn[0]) SDL_BlitSurface(sm->retrn_btn[0], NULL, ecran, &sm->pos_btn_retrn);
    }
}
